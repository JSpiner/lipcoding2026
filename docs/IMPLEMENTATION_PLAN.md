...
음성 인식 결과에서 오타, 잘못된 한/영 변환, 도메인 용어 오인식을 빠르게 교정해 사용자가 체감하는 반응속도를 높인다. 이 단계는 코어 데모를 해치지 않는 후속 고도화로 두고, 실패 시에도 기존 명령 경로가 즉시 동작해야 한다.

### 구현 범위

- Speech-to-Text 결과에 대한 경량 정규화와 도메인 사전 보정
- 모호한 경우에만 Copilot SDK 또는 Azure OpenAI 기반 교정 요청
- 교정 전/후 비교 프리뷰 UI 표시
- 신뢰도에 따른 자동 적용 / 사용자 승인 / 원문 유지 분기
- 짧은 타임아웃과 캐시를 둔 저지연 폴백 경로
...
- 1차 규칙 교정: 자주 틀리는 음성 인식 패턴을 즉시 치환한다.
- 2차 LLM 교정: 저신뢰 구간에서만 JSON 형식의 교정 제안을 요청한다.
- 자동 적용 기준: 신뢰도가 높고 변경 범위가 좁을 때만 원문을 대체한다.
- 사용자 확인 기준: 교정 전/후 차이가 크거나 삭제/위험 명령이 포함되면 승인 UI를 띄운다.
- 타임아웃 기준: 교정 요청이 늦어지면 원문 또는 규칙 교정 결과를 바로 실행한다.

### 완료 기준

- 음성 입력에서 오타와 한/영 오인식이 눈에 띄게 줄어든다.
- 교정 전/후를 비교해 보고 원문 유지 또는 적용을 즉시 선택할 수 있다.
- 교정 단계가 지연될 때도 명령 실행이 멈추지 않는다.
- 교정 성공률과 폴백률을 확인할 수 있다.

### 이 단계의 리스크

- 교정 단계가 길어지면 전체 반응속도가 느려진다.
- LLM이 원래 의도를 바꾸는 과교정이 발생할 수 있다.
- 도메인 사전이 부족하면 반복 오류가 남는다.

### 리스크 대응

- 규칙 교정을 기본으로 두고 LLM 교정은 보조로 사용한다.
- 교정 요청에는 짧은 타임아웃을 걸고 초과 시 즉시 폴백한다.
- 교정 전/후 비교 UI로 사용자가 최종 통제권을 갖게 한다.
...
| 4단계 | 음성 교정 프리뷰, 저지연 폴백, 신뢰도 기반 적용 | 과도한 자동 보정, 세부 분석 대시보드 |
